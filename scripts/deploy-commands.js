...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(opt => opt.setName('boton1').setDescription('Etiqueta del botón 1').setRequired(false))
    .addStringOption(opt => opt.setName('pregunta1').setDescription('Pregunta del formulario para el botón 1 (opcional)').setRequired(false))
    .addStringOption(opt => opt.setName('boton2').setDescription('Etiqueta del botón 2').setRequired(false))
    .addStringOption(opt => opt.setName('pregunta2').setDescription('Pregunta del formulario para el botón 2 (opcional)').setRequired(false))
    .addStringOption(opt => opt.setName('boton3').setDescription('Etiqueta del botón 3').setRequired(false))
  ...
  new SlashCommandBuilder()
    .setName('automod')
    .setDescription('Configurar la moderación automática del servidor')
    .addSubcommand(sub => sub
      .setName('role')
      .setDescription('Configura el rol autorizado para gestionar automoderación')
      .addRoleOption(opt => opt.setName('rol').setDescription('Rol autorizado').setRequired(true)))
    .addSubcommand(sub => sub
      .setName('settings')
      .setDescription('Actualizar opciones de moderación automática')
      .addBooleanOption(opt => opt.setName('antispam').setDescription('Activar o desactivar anti-spam').setRequired(false))
      .addBooleanOption(opt => opt.setName('antilinks').setDescription('Activar o desactivar anti-links').setRequired(false))
      .addBooleanOption(opt => opt.setName('antibots').setDescription('Activar o desactivar anti-bots').setRequired(false))
      .addBooleanOption(opt => opt.setName('antichannelspam').setDescription('Activar o desactivar anti-creación/eliminación de canales').setRequired(false))
      .addIntegerOption(opt => opt.setName('maxmessages').setDescription('Máximo mensajes en la ventana de tiempo').setMinValue(1).setMaxValue(20).setRequired(false))
      .addIntegerOption(opt => opt.setName('timewindow').setDescription('Ventana de tiempo en segundos').setMinValue(1).setMaxValue(60).setRequired(false)))
    .addSubcommand(sub => sub
      .setName('status')
      .setDescription('Ver estado actual de la moderación automática'))
    .toJSON(),
  new SlashCommandBuilder()
    .setName('logs')
